                            ...
                            if path.parent().map(|p| p.ends_with("inbox")).unwrap_or(false) {
                                if fname.ends_with(".json") || fname.ends_with(".md") {
                                    if let Ok(raw) = std::fs::read_to_string(path) {
                                        if let Ok(signed) = serde_json::from_str::<crate::crypto::SignedMessage>(&raw) {
                                            let verified = crate::crypto::verify_message(&signed);
                                            let wrapped = crate::crypto::wrap_external_message(&signed, verified);
